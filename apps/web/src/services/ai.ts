...
type AIRequestPayload = Record<string, unknown>;
...
export const aiService = {
  ...
  summarize: async (
    ...
    if (onChunk)
      return aiService.fetchSSE("/ai/summarize", { noteId, mode }, onChunk);
    const { data } = await api.post<AIResult>("/ai/summarize", {
      noteId,
      mode,
    });
    return data.result;
  ...
  continueWriting: async (
    ...
    if (onChunk)
      return aiService.fetchSSE("/ai/continue", { content }, onChunk);
  ...
  rewrite: async (
    ...
    if (onChunk)
      return aiService.fetchSSE("/ai/rewrite", { content, style }, onChunk);
    const { data } = await api.post<AIResult>("/ai/rewrite", {
      content,
      style,
    });
    return data.result;
  ...
  suggestTags: async (
    ...
    if (onChunk)
      return aiService.fetchSSE("/ai/suggest-tags", { noteId }, onChunk);
    const { data } = await api.post<AIResult>("/ai/suggest-tags", { noteId });
    return data.result;
  },

  // Sprout: Find related notes using semantic search
  sprout: async (noteId: string, content: string): Promise<unknown[]> => {
    const { data } = await api.post<{ results: unknown[] }>("/ai/sprout", {
      noteId,
      content,
    });
    return data.results || [];
