...
export function BlockHoverControls({ editor }: BlockHoverControlsProps) {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (
        ...
        const domNode = view.nodeDOM(
          resolvedPos.before(resolvedPos.depth),
        ) as HTMLElement;
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const insertionPos = pendingPicker?.kind === "image" ? pendingPicker.pos : null;
    setPendingPicker(null);
    if (!file || insertionPos === null) return;
    try {
      const url = await uploadFile(file);
      editor
        ?.chain()
        .focus()
        .setNodeSelection(insertionPos)
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    const insertionPos = pendingPicker?.kind === "file" ? pendingPicker.pos : null;
    setPendingPicker(null);
    if (!file || insertionPos === null) return;
    try {
      const url = await uploadFile(file);
      editor
        ...
        .insertContent({
          ...
          marks: [
            {
              type: "link",
              attrs: { href: url, download: true, target: "_blank" },
            },
          ],
        })
        .run();
  ...
  useEffect(() => {
    if (pendingPicker?.kind === "image") imgInputRef.current?.click();
    if (pendingPicker?.kind === "file") fileInputRef.current?.click();
  }, [pendingPicker]);

  const openImagePicker = useCallback(() => {
    if (!hoverState) return;
    setIsMenuOpen(false);
    setPendingPicker({ kind: "image", pos: hoverState.pos });
  }, [hoverState]);

  const openFilePicker = useCallback(() => {
    if (!hoverState) return;
    setIsMenuOpen(false);
    setPendingPicker({ kind: "file", pos: hoverState.pos });
  }, [hoverState]);
        ...
        >
          ...
          >
            <button
              ...
              title="H1"
            >
              <Heading1 size={16} color="var(--text-secondary)" />
            </button>
            <button
              onClick={() =>
                insertBlock(() =>
              ...
              title="H2"
            >
              <Heading2 size={16} color="var(--text-secondary)" />
            </button>
            <button
              onClick={() =>
                insertBlock(() =>
              ...
              title="H3"
            >
              <Heading3 size={16} color="var(--text-secondary)" />
            </button>
            <button
              onClick={() =>
                insertBlock(() =>
              ...
              title="H4"
            >
              <Heading4 size={16} color="var(--text-secondary)" />
            </button>
            <button
              onClick={() =>
                insertBlock(() =>
              ...
              title="H5"
            >
              <Heading5 size={16} color="var(--text-secondary)" />
            </button>
            ...
            <button
              onClick={() =>
                insertBlock(() => editor.chain().focus().toggleTaskList().run())
              }
              style={{
                background: "transparent",
                border: "none",
            ...
            <button
              ...
              title="引用"
            >
              <Quote size={16} color="var(--text-secondary)" />
            </button>
            <button
              onClick={() =>
                insertBlock(() =>
          ...
          {renderMenuItem(<TableIcon size={15} />, "表格", () =>
            editor
              .chain()
              .focus()
          ...
          {renderMenuItem(<Code size={15} />, "代码块", () =>
          ...
          {renderMenuItem(<Minus size={15} />, "分割线", () =>
            editor.chain().focus().setHorizontalRule().run(),
          )}
          ...
          {renderMenuItem(
            <FileVideo size={15} color="#00A1D6" />,
            "哔哩哔哩",
            () => {},
          )}
          {renderMenuItem(
            <FileVideo size={15} color="#FF3A3A" />,
            "墨刀",
            () => {},
          )}
          {renderMenuItem(
            <FileVideo size={15} color="#FF9900" />,
            "腾讯视频",
            () => {},
          )}
        </div>
