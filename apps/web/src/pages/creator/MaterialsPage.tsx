...
import { ArrowRight, ExternalLink, FilePlus2, Link2, Loader2, Search } from 'lucide-react';
...
export default function MaterialsPage() {
  ...
  return <div className="creator-page materials-page">
    ...
    {items.length===0?<div className="materials-empty"><div>还没有素材</div><p>粘贴上方的网址，或手写第一条。积累从一条真实材料开始。</p></div>:<div className="materials-grid">{items.map(item => <article key={item.id} className="material-card"><div className="material-card-top"><span className={`source-badge source-${item.sourceType||'manual'}`}>{item.sourceType==='web'?'网页':item.sourceType==='audio'?'语音':'手写'}</span>{item.sourceUrl&&<a href={item.sourceUrl} target="_blank" rel="noreferrer" aria-label="打开原始链接"><ExternalLink size={15}/></a>}</div><h3>{item.title || '无标题素材'}</h3><p>{item.contentText || item.transcript || '暂无文字内容'}</p>{insights[item.id]?.map((x,i)=><div key={i} className="insight-item"><b>{x.type}</b> {x.content}</div>)}<div className="material-card-actions"><button onClick={() => navigate(`/n/${item.id}`)}>查看原文</button><button onClick={()=>extract(item.id)}>AI 提炼</button></div></article>)}</div>}
  </div>;
}
